...
describe("boot fixtures", () => {
    ...
    it("merges labs overrides without mutating the canned data", () => {
        ...
        expect(getLabs(overridden)).toMatchObject({editorExcerpt: true, additionalPaymentMethods: false});
        expect(getLabs(settingsResponse())).toMatchObject({editorExcerpt: false});
        expect(configResponse({labs: {editorExcerpt: true}}).config.labs).toMatchObject({editorExcerpt: true});
    });
