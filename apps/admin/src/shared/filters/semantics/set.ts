...
// A set writes `$in` / `$nin` but must also read a bare `$eq` / `$ne` back, because a
// single-value selection is written that way by older filters. More readings than
// writings, so the table stays one-directional here too.
...
const SET_SYMBOLS: Partial<Record<OperatorId, string>> = SET_OPERATOR_SYMBOLS;
