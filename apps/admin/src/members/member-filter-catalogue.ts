import {buildProvidedCatalogue, catalogueCanRead} from '@/shared/filters';
import {customFieldProvider, type CustomFieldDefinition} from './custom-field-filter-fields';
...
//
// The static catalogue already answers for every field, including the parameterised entries
...
export interface MemberCatalogueSources {
    newsletters?: readonly NewsletterDefinition[];
    customFields?: readonly CustomFieldDefinition[];
}

/**
 * The providers behind a catalogue, in resolution order.
 *
 ...
 */
export function memberFieldProviders({newsletters, customFields}: MemberCatalogueSources = {}): FieldProvider[] {
    return [
...
export function buildMemberFields(sources: MemberCatalogueSources = {}): MemberFields {
    if (!sources.newsletters && !sources.customFields) {
        return memberFields;
    }

    return buildProvidedCatalogue(memberFieldProviders(sources));
}
 ...
 */
export function canReadMemberFilter(filter: string | undefined, sources: MemberCatalogueSources = {}): boolean {
    return catalogueCanRead(filter, memberFieldProviders(sources));
}
