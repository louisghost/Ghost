...
export function applyFakedUserEdit(body: unknown): CurrentUser {
...
export function defaultBootRequests() {
    return {
        ...
        editUserPreferences: {
            ...
            response: async (request: Request) => applyFakedUserEdit(await request.clone().json()),
        },
...
export function fakePreferenceEdits(): EndpointCapture {
    return fakeAdminEndpoint("PUT", /^\/users\/\w+\/\?include=roles/, ({body}) => applyFakedUserEdit(body));
}
