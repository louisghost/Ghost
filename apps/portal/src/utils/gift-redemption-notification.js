...
export function getGiftIntroduction({buyerName, cadence, duration, siteTitle} = {}) {
    ...
    if (buyerName && siteTitle) {
        return t('{buyerName} has gifted you a {duration}-month {tierName} membership to {siteTitle}', {count: duration});
    }
    if (siteTitle) {
        return t('You\'ve been gifted a {duration}-month {tierName} membership to {siteTitle}', {count: duration});
    }
