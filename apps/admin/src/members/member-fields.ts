...
import {FILTER_TYPES, type FieldDescriptor, buildCatalog, domainField, columnAddressing, withFutureRelativeOperator, withPastRelativeOperator} from '@/shared/filters';
...
// column. Neither half is written here: the grammar is in custom-fields/addressing.ts and
...
function withRelativeOperators(descriptor: FieldDescriptor): FieldDescriptor {
    if (descriptor.relative === 'past') {
        return withPastRelativeOperator(descriptor);
    }

    if (descriptor.relative === 'future') {
...
const CUSTOM_FIELD: FieldDescriptor = {
    key: 'custom_field.:key', icon: 'text',
    type: 'text',
 ...
 * over these — see member-filter-catalog.ts — but these alone can read any filter.
 */
